...
export default function RecordingPage() {
    ...
    useEffect(() => {
        const newClient = new RealtimeClient({
            apiKey: "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",
        ...
        const initializeClient = async () => {
            await newClient.updateSession({
                instructions: "Listen to the user's argument and provide a question for their position. Be direct and concise. Your response should be no longer than 2-3 sentences.",
                voice: 'alloy',
    ...
    useEffect(() => {
        ...
        const handleConversationUpdate = async (event) => {
            ...
            if(item.type === 'message' && item.role === 'user' && item.status === 'completed') {
                localStorage.setItem('history', 'User: ' + localStorage.getItem('history') + item.formatted.transcript + '\n');
            }
            ...
            if (item.type === 'message' && item.role === 'assistant') {
                if (item.status === 'completed') {
                    ...
                    localStorage.setItem('history', 'Assistant: ' + localStorage.getItem('history') + item.formatted.transcript + '\n');
        ...
        try {
            ...
            processor.onaudioprocess = (e) => {
                ...
                if (audioChunks.length >= 4) {
                    const combinedChunks = new Int16Array(audioChunks.reduce((acc, chunk) => acc + chunk.length, 0));
                    let offset = 0;
